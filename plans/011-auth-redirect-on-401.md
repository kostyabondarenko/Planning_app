...
**Результат:**
- [x] При 401 токен удаляется из localStorage
- [x] Происходит redirect на /login с параметром redirect
- [x] Ошибка AUTH_EXPIRED выбрасывается для корректной обработки в хуках
- [x] Остальные ошибки не затронуты
...
**Результат:**
- [x] useCalendar не показывает ошибку при AUTH_EXPIRED
- [x] useTasks не показывает ошибку при AUTH_EXPIRED
- [x] useGoals не показывает ошибку при AUTH_EXPIRED (убрана старая проверка 401)
- [x] Все остальные хуки обновлены (useGoal, useMilestone)
- [x] Ошибки не связанные с авторизацией продолжают отображаться
...
**Результат:**
- [x] Страница логина читает параметр redirect из URL
- [x] После входа перенаправляет на redirect (или /dashboard по умолчанию)
- [x] Защита от open redirect (только относительные пути)
- [x] Сообщение "Сессия истекла" при наличии redirect
...
**Результат:**
- [x] Все сценарии проверены
- [x] Все защищённые страницы корректно обрабатывают 401
...
### Функциональность
- [x] При 401 происходит redirect на /login
- [x] Стейлый токен удаляется из localStorage
- [x] Текущий путь сохраняется в redirect-параметре
- [x] После логина — возврат на сохранённую страницу
- [x] "Попробовать снова" работает для не-401 ошибок

### Безопасность
- [x] Защита от open redirect (только относительные пути + проверка на //)
- [x] SSR-совместимость (typeof window !== 'undefined')

### Качество
- [x] Нет TypeScript ошибок
- [x] Нет console errors
- [x] Документация обновлена (architecture.md)
